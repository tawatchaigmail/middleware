const model = require('../models/reportFileParameters.model');

 module.exports = reportFileParametersController = {
  ...
  create : async (req, res, next) => {
    try {
     ...
     if (dataCreate) {
       // res.status(200).json({updated: dataCreate})
       res.json(dataCreate);   
     } else {
  ...
  delete : async (req, res, next) => {
    try {
      ...
      if (dataDelete) {
     //  res.status(200).json({delete: dataDelete})
       res.json(dataDelete);   
      } else {
        res.status(404).json({message: "Record not found"});
      }        
